 ...
 */
export default function SplitScrolly({ beats, onBeatChange, children, className }: Props) {
        ...
        <div className="w-full md:w-[45%]">
          ...
          {beats.map((beat, i) => (
            <div
              ...
              className={`mb-[50vh] transition-opacity duration-700 ${
                i <= activeIndex ? "opacity-100" : "opacity-[0.08]"
              }`}
            >
              <div className="text-lg font-bold leading-snug text-white sm:text-xl">
                {beat.text}
              </div>
            </div>
          ))}
