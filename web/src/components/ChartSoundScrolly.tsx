...
export default function ChartSoundScrolly({
  songFeatures,
  eraAverage,
  songName,
  beat,
  ...
  useEffect(() => {
    ...
    if (isFirstDraw) {
      ...
      deltas.slice(0, 2).forEach(({ feat, index, songVal, eraVal }, ci) => {
        ...
        // Highlight ring around the data point
        g.append("circle")
        ...
        const pctDiff =
          eraVal > 0 ? Math.round(Math.abs(diff / eraVal) * 100) : 0
        const direction = diff > 0 ? "above" : "below"

        // Position label outside the radar chart, below the axis label
        const labelR = radius + 24
        const baseLx = Math.cos(angle) * labelR
        const baseLy = Math.sin(angle) * labelR
        // Shift down 18px so callout sits below the axis label
        const lx = baseLx
        const ly = baseLy + 40
        const anchor =
        ...
        // Dashed leader line from data point to label
        g.append("line")
          .attr("class", "sound-callout")
          .attr("x1", px)
          .attr("y1", py)
    ...
    if (beat >= 1) {
      if (shouldAnimate && prevBeat < 1) {
        g.select(".sound-era-poly")
          .transition()
          .duration(600)
          .attr("fill-opacity", 0.08)
          .attr("stroke-opacity", 0.6)
        g.selectAll(".sound-era-dot")
          .transition()
          .delay(300)
          .duration(300)
          .attr("opacity", 1)
    ...
    if (beat >= 2) {
      if (shouldAnimate && prevBeat < 2) {
        g.select(".sound-song-poly")
          .transition()
          .duration(800)
          .attr("fill-opacity", 0.12)
          .attr("stroke-opacity", 1)
        g.selectAll(".sound-song-dot")
          .transition()
          .delay(400)
          .duration(400)
          .attr("opacity", 1)
    ...
    <div>
      <div className="flex justify-center">
        <svg
          ref={svgRef}
          role="img"
          aria-label={`Radar chart comparing ${songName} audio features against era average`}
        />
      </div>
    </div>
  )
